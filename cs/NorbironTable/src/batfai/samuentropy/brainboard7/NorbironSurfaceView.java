...
class Nodes {
    ...
    public static android.graphics.Bitmap menuButton;
    ...
    public Nodes(android.view.SurfaceView surfaceView) {
        ...
        resId = surfaceView.getResources().getIdentifier("menu", "drawable",
                "batfai.samuentropy.brainboard7");
        menuButton = android.graphics.BitmapFactory.decodeResource(surfaceView.getResources(), resId);
...
public class NorbironSurfaceView extends android.view.SurfaceView implements Runnable {
    ...
    private static final long doublePressms = 200;
   ...
   protected static java.util.List<NeuronBox> nodeBoxes = new java.util.ArrayList<NeuronBox>();
    ...
    private byte tappedBuildButton = 0;
    ...
    public static void saveData(android.content.SharedPreferences.Editor editor)
    {
        ...
        for(NeuronBox nb : nodeBoxes)
        {
            ...
            editor.putInt("type"+i, nb.nodeType);
  ...
  public static void loadData(android.content.SharedPreferences settings)
    {
        ...
        {
            int nodeType = settings.getInt("type"+i, 0);
            NeuronBox box = (NeuronBox)Nodes.get(nodeType).clone();
            nodeBoxes.add(box);
    ...
    public void onDraw(android.graphics.Canvas canvas) {
        ...
        if (tappedBuildButton == 0)
            {
            	canvas.drawBitmap(nodes.menuButton, null, new android.graphics.Rect(0, 0, 100, 100), null);}
    ...
    public float d(float x1, float x2, float y1, float y2) {
     ...
     protected int nearestNeuronBox(float x, float y) {
    ...
    public boolean onTouchEvent(android.view.MotionEvent event) {
       ...
       if (event.getX() < 100 && event.getY() < 100)
            ...
            if (pressTime - Pressed >= doublePressms)
                doubleTap = false;
            else
                doubleTap = true;
            Pressed = pressTime;
            ...
            int nearestNB = nearestNeuronBox(x + startsx, y + startsy);
            NeuronBox nb = null;
